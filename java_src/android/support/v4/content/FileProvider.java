...
public class FileProvider extends ContentProvider {
    ...
    private static final File DEVICE_ROOT = new File(CameraUtilsEffects.FILE_DELIM);
    ...
    public static class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            ...
            try {
                ...
                if (rootPath2.endsWith(CameraUtilsEffects.FILE_DELIM)) {
                    path = path2.substring(rootPath2.length());
                } else {
                    path = path2.substring(rootPath2.length() + 1);
                }
                return new Uri.Builder().scheme(UriUtil.LOCAL_CONTENT_SCHEME).authority(this.mAuthority).encodedPath(Uri.encode(mostSpecific.getKey()) + '/' + Uri.encode(path, CameraUtilsEffects.FILE_DELIM)).build();
            } catch (IOException e) {
