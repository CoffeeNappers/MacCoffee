...
public abstract class Flowable<T> implements Publisher<T> {
    ...
    public final <U extends Collection<? super T>> Flowable<U> buffer(int count, int skip, Callable<U> bufferSupplier) {
        ObjectHelper.verifyPositive(count, ServerKeys.COUNT);
        ObjectHelper.verifyPositive(skip, FreeSpaceBox.TYPE);
    ...
    public final <R> R to(Function<? super Flowable<T>, R> converter) {
        try {
            return (R) ((Function) ObjectHelper.requireNonNull(converter, "converter is null")).mo1190apply(this);
        } catch (Throwable ex) {
