...
public abstract class Maybe<T> implements MaybeSource<T> {
    ...
    public static <T> Flowable<T> mergeArrayDelayError(MaybeSource<? extends T>... sources) {
        return sources.length == 0 ? Flowable.empty() : Flowable.fromArray(sources).flatMap(MaybeToPublisher.instance(), true, sources.length);
    }
    ...
    public final <R> R to(Function<? super Maybe<T>, R> convert) {
        try {
            return (R) ((Function) ObjectHelper.requireNonNull(convert, "convert is null")).mo1190apply(this);
        } catch (Throwable ex) {
