...
public abstract class ParallelFlowable<T> {
    ...
    public static <T> ParallelFlowable<T> from(@NonNull Publisher<? extends T> source, int parallelism, int prefetch) {
        ObjectHelper.requireNonNull(source, ShareConstants.FEED_SOURCE_PARAM);
    ...
    public final <U> U to(@NonNull Function<? super ParallelFlowable<T>, U> converter) {
        try {
            return (U) ((Function) ObjectHelper.requireNonNull(converter, "converter is null")).mo1190apply(this);
        } catch (Throwable ex) {
