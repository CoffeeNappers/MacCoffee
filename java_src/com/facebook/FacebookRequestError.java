...
public final class FacebookRequestError {
    ...
    public static FacebookRequestError checkResponseAndCreateError(JSONObject singleResult, Object batchResult, HttpURLConnection connection) {
        try {
            if (singleResult.has(AuthCheckFragment.KEY_CODE)) {
                int responseCode = singleResult.getInt(AuthCheckFragment.KEY_CODE);
                ...
                if (body != null && (body instanceof JSONObject)) {
                    ...
                    if (jsonBody.has("error")) {
                        ...
                        errorCode = error.optInt(AuthCheckFragment.KEY_CODE, -1);
                ...
                if (!HTTP_RANGE_SUCCESS.contains(responseCode)) {
                    return new FacebookRequestError(responseCode, -1, -1, null, null, null, null, false, singleResult.has("body") ? (JSONObject) Utility.getStringPropertyAsJSON(singleResult, "body", GraphResponse.NON_JSON_RESPONSE_PROPERTY) : null, singleResult, batchResult, connection, null);
                }
            }
