...
public class GetStickerStockItemById extends VKAPIRequest<StickerStockItem> {
    ...
    public GetStickerStockItemById(int id) {
    ...
    public StickerStockItem mo1093parse(JSONObject source) throws Exception {
        JSONArrayWithCount stickers = APIUtils.unwrapArray(source, ServerKeys.RESPONSE);
        StickerStockItem item = new StickerStockItem(stickers.array.getJSONObject(0), 0);
        ArrayList arrayList = new ArrayList(1);
        arrayList.add(item);
        Stickers.get().fillLocalInfo(arrayList, new F1<StickerStockItem, StickerStockItem>() { // from class: com.vkontakte.android.api.store.GetStickerStockItemById.1
